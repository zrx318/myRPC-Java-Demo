...
public class RegisterCenter implements IRegisterCenter4Invoker, IRegisterCenter4Provider, IRegisterCenter4Governance {
    ...
    public void registerProvider(final List<ProviderService> serviceMetaData) {
        ...
        synchronized (RegisterCenter.class) {
            String serviceItfKey = serviceMetaData.get(0).getServiceItf().getName();
            ...
            // key值为相应的接口，注意这里虽然key值对应的是一个list，但是一个key对应的value只会在注册中心注册一次
            //服务分组
            String groupName = serviceMetaData.get(0).getGroupName();
            //创建服务提供者
            ...
            //创建当前服务器节点
            //服务端口
            int serverPort = serviceMetaData.get(0).getServerPort();
            //服务权重
            int weight = serviceMetaData.get(0).getWeight();
            //服务工作线程
            int workerThreads = serviceMetaData.get(0).getWorkerThreads();
            String localIp = IPHelper.localIp();
            String currentServiceIpNode = servicePath + "/" + localIp + "|" + serverPort + "|" + weight + "|" + workerThreads + "|" + groupName;
            System.out.println("currentServiceIpNode: " + currentServiceIpNode);
            ...
            //System.out.println("providerServiceMap: " + providerServiceMap.size());
     ...
     *  TODO : 有bug，同一台主机上（ip地址相同），两个不同端口上开两个服务，这里是察觉不到的
     */
    ...
    private void refreshServiceMetaDataMap(List<String> serviceIpList) {
        ...
        NettyChannelFactory.channelFactoryInstance().initChannelPoolFactory(serviceMetaDataMap4Consume);
    }
